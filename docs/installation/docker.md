...
- 已安装 [Docker](https://docs.docker.com/get-docker/) **24.0+**（内置 Compose V2 插件）
- 验证方式：`docker compose version` 输出应为 `v2.x.x` 或更高

> Docker 24.0+ 已将 Compose V2 作为内置插件随 Docker 一同安装，无需单独安装。  
> 如系统 Docker 版本过旧，可通过官方脚本升级：`curl -fsSL https://get.docker.com | sh`
...
**Q：运行 `docker compose` 提示命令不存在或版本过低？**
升级 Docker 到 24.0+ 即可，Compose V2 已作为内置插件随附：
```bash
